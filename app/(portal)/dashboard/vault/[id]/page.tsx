...
export default function EvidenceDetailPage() {
    ...
    const [incidentType, setIncidentType] = useState<'physical' | 'emotional' | 'financial' | 'sexual' | 'other'>("physical");
    ...
    const updateDetails = async () => {
        ...
        if (success) {
            setItem({ ...item, description, threat_level: threatLevel, incident_type: incidentType });
    ...
    const handleDownload = async () => {
        ...
        const warning = t('download_confirm') || "WARNING: This file will be saved to your device unencrypted.\\n\\nAre you sure?";
        ...
        try {
            ...
            const filename = item.file_id.split('/').pop() || `evidence-${Date.now()}.bin`;
    ...
    const getViewUrl = (fileId: string) => {
        return supabase.storage.from('evidence-vault').getPublicUrl(fileId).data.publicUrl;
    };

    if (loading) return <div className="h-screen bg-black flex items-center justify-center"><Spinner color="white" /></div>;
    if (!item) return null;
        ...
        <div className="min-h-screen bg-black pb-12 pt-[calc(env(safe-area-inset-top)+20px)] px-4">
            ...
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="text-right">
                   ...
                   <div className="flex items-center justify-end gap-2 text-[10px] text-zinc-600 font-mono uppercase">
                        <span>{item.incident_type ? t(`incident_type_${item.incident_type}` as any) : t('detail_secure')}</span>
                        <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${Number(item.threat_level) > 3 ? 'bg-red-500' : 'bg-green-500'}`}/>
                   </div>
                </div>
            </div>
            ...
            <div className="max-w-md mx-auto space-y-8">
                ...
                <div className="relative">
                    {!isCall ? (
                        <div className="w-full bg-zinc-900/30 border border-zinc-800 rounded-2xl overflow-hidden min-h-[260px] flex items-center justify-center relative group">
                             <div className="absolute top-3 left-3 z-10">
                                 <span className="px-2 py-1 rounded bg-black/50 backdrop-blur text-[10px] text-zinc-400 font-mono border border-white/10">
                                    {isAudio ? t('detail_audio_record') : t('detail_image_capture')}
                                 </span>
                             </div>
                             ...
                             <div className="absolute top-3 right-3 z-10 flex gap-2">
                                <Button
                                    ...
                                    size="sm"
                                    variant="flat" 
                                    className="bg-black/50 backdrop-blur text-white border border-white/10 hover:bg-black/70"
                             ...
                             {isAudio ? (
                                 <div className="flex flex-col items-center gap-6 w-full p-8">
                                    ...
                                    <audio controls className="w-full" src={getViewUrl(item.file_id)} />
                                 </div>
                             ) : (
                                 <div className="w-full h-full flex flex-col items-center justify-center relative">
                                 {isImageLoading && !secureImageSrc && (
                                 ...
                                 <img 
                                    ...
                                    alt="Evidence"
                                    className={`w-full h-full object-contain max-h-[400px] cursor-pointer transition-opacity duration-300 ${isImageLoading ? 'opacity-0' : 'opacity-100'}`}
                                    onClick={() => window.open(getViewUrl(item.file_id), '_blank')}
                                 />
                             </div>
                             )}
                        </div>
                        ...
                        <div className="w-full bg-gradient-to-br from-blue-900/10 to-zinc-900 border border-blue-500/10 rounded-2xl p-8 flex flex-col items-center justify-center gap-4 min-h-[180px]">
                            ...
                            <h3 className="text-blue-400/80 font-mono text-sm tracking-widest text-center">{t('detail_sos_log')}</h3>
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-4">
                    <h3 className="text-zinc-500 text-xs font-bold uppercase tracking-widest pl-1">{t('detail_intelligence_title')}</h3>
                    
                    <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5 space-y-6">
                        <div className="grid grid-cols-2 gap-y-6 gap-x-4">
                             <div>
                                 <label className="text-[10px] text-zinc-500 uppercase font-bold mb-1 block">{t('detail_date')}</label>
                             ...
                             <div>
                                 <label className="text-[10px] text-zinc-500 uppercase font-bold mb-1 block">{t('detail_time')}</label>
                                 <div className="flex items-center gap-2 text-zinc-300 text-sm font-mono">
                                     <span className="opacity-70">{new Date(item.timestamp).toLocaleTimeString()}</span>
                                 </div>
                             </div>
                             
                             <div className="col-span-2 space-y-2">
                                <label className="text-[10px] text-zinc-500 uppercase font-bold block">{t('detail_incident_class')}</label>
                             ...
                             <div className="col-span-2">
                                 <label className="text-[10px] text-zinc-500 uppercase font-bold mb-2 block">{t('detail_threat_assess')}</label>
                                 <ThreatSelector />
                             </div>
                        </div>

                        <div className="pt-4 border-t border-zinc-800/50">
                            <label className="text-[10px] text-zinc-500 uppercase font-bold mb-2 block">{t('detail_notes_label')}</label>
                            <Textarea 
                                value={description} 
                                ...
                                classNames={{
                                    ...
                                    inputWrapper: "bg-zinc-950 border-zinc-800 hover:border-zinc-700 group-data-[focus=true]:border-zinc-600"
                                }}
                            />
                        </div>
                        ...
                        {!isCall && (
                            ...
                            >
                                <div className="flex flex-col items-start gap-0.5">
                                    <span className="text-xs font-bold tracking-wider uppercase">{t('detail_purge_file_btn')}</span>
                                    <span className="text-[10px] text-zinc-600 font-mono group-hover:text-red-400/60">{t('detail_purge_file_sub')}</span>
                                </div>
                            </Button>
                        )}

                        <Button 
                           className="w-full justify-center bg-red-950/20 border border-red-900/30 text-red-600 font-bold h-12" 
                           variant="flat"
