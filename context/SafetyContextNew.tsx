  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      try {
        ...
        console.log("[SafetyContext] No active session (expected on first load)");
      }
  ...
  const triggerAuthFlow = async (pin: string): Promise<boolean> => {
    ...
    try {
      ...
      try {
        ...
        console.log("[SafetyContext] User:", { id: user.$id, email: user.email });
        ...
        console.log(`[SafetyContext] Login failed: ${loginError.message}`);
        ...
        if (
          ...
          try {
            ...
            console.log("[SafetyContext] Creating new account...");
            ...
            console.log("[SafetyContext] Account created, logging in...");
            await account.createEmailPasswordSession(email, password);
            await new Promise((r) => setTimeout(r, 200));

            user = await account.get();
            console.log("[SafetyContext] ✅ NEW ACCOUNT CREATED");
            ...
            if (
              ...
              console.error("[SafetyContext] ❌ Wrong PIN");
      ...
      if (user) {
        ...
        try {
          ...
          if (prefs.pref_language && ["en", "am", "om"].includes(prefs.pref_language)) {
          ...
          console.warn("[SafetyContext] Could not fetch preferences:", prefsError);
        }
