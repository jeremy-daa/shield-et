...
export const CamouflageHeader = () => {
  ...
  const handlePointerDown = (e: React.SyntheticEvent) => {
    ...
    timerRef.current = setTimeout(() => {
      console.log("Long Press Complete - Opening PIN Modal");
      onOpen(); // NEW: Just open modal directly, no auth trigger
      setIsPressing(false);
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-background/80 backdrop-blur-md border-b border-zinc-800/50">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        >
          <div
            className={`transition-all duration-300 ${isPressing ? "opacity-50 scale-95" : "opacity-100"}`}
          >
            <h1 className="text-xl font-bold tracking-tighter flex items-center gap-2">
              <span className="w-8 h-8 bg-foreground text-background flex items-center justify-center rounded-lg font-serif transition-colors">
                S
              </span>
              <span>Shield News Portal</span>
            </h1>
          </div>
        </div>
