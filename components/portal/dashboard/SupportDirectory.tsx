...
export default function SupportDirectory() {
  ...
  // Mock Data for Dev/Visuals
  const MOCK_RESOURCES: Partial<SupportResource>[] = [
      {
          $id: 'mock1',
          name: "Addis Women's Shelter",
          category: 'shelter',
          phone: "+251911000000",
          location: "Addis Ababa",
          description_en: "Safe haven for women and children offering temporary housing and counseling.",
          description_am: "ለሴቶች እና ለህጻናት ጊዜያዊ መጠለያ እና የምክር አገልግሎት የሚሰጥ አስተማማኝ ቦታ።",
          description_or: "Bakka jireenya yeroo fi gorsa dubartootaa fi daa'immaniif kennu.",
          verified: true,
          lat: 9.0306,
          lng: 38.7469
      },
      {
          ...
          phone: "+251911223344",
          location: "Addis Ababa",
          description_en: "Free legal counseling and representation for survivors of violence.",
          description_am: "ለጥቃት ሰለባዎች ነፃ የሕግ ምክር እና ወኪልነት።",
          description_or: "",
          verified: true,
          lat: 9.0400,
          lng: 38.7500
      ...
      {
          $id: 'mock3',
          name: "St. Paul's Trauma Unit",
          category: 'medical',
          phone: "907",
          location: "Addis Ababa",
          description_en: "Emergency medical services and sexual assault response team (SART).",
          description_am: "የአደጋ ጊዜ የህክምና አገልግሎቶች እና የወሲብ ጥቃት ምላሽ ቡድን።",
          description_or: "",
          verified: true,
          lat: 9.0500,
          lng: 38.7300
      ...
      {
          $id: 'mock4',
          name: "Dire Dawa Crisis Center",
          category: 'counseling',
          phone: "+25125111111",
          location: "Dire Dawa",
          description_en: "24/7 Crisis intervention and psychological support.",
          description_am: "24/7 የችግር ጣልቃ ገብነት እና ስነ-ልቦናዊ ድጋፍ።",
          description_or: "",
          verified: true,
          lat: 9.6009,
          lng: 41.8501
    ...
    const fetch = async () => {
      try {
        ...
        if (dbDocs.length > 0) {
             ...
             setResources(MOCK_RESOURCES as SupportResource[]);
        }
      } catch (e) {
        console.error(e);
        setResources(MOCK_RESOURCES as SupportResource[]);
  ...
  const selectedCity = useMemo(() => {
    return SEARCH_INDEX.find(c => c.n === selectedCityKey) || SEARCH_INDEX.find(c => c.n === "Addis Ababa");
  }, [selectedCityKey]);
  ...
  const sortedResources = useMemo(() => {
    ...
    return [...resources].sort((a, b) => {
      const distA = calculateDistance(activeLocation.lat, activeLocation.lng, a.lat || 0, a.lng || 0);
      const distB = calculateDistance(activeLocation.lat, activeLocation.lng, b.lat || 0, b.lng || 0);
    ...
    <div className="flex flex-col gap-8 p-5 pb-32 min-h-screen">
       ...
       <div className="flex flex-col gap-2 mt-2">
           <div className="flex items-center gap-2">
               ...
               <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">{t('nav_support')}</span>
           </div>
           <div>
               <h1 className="text-3xl font-bold text-white tracking-tight">{t('support_title')}</h1>
               <p className="text-sm text-zinc-400 mt-1 max-w-[280px]">{t('support_subtitle')}</p>
           </div>
       </div>
       ...
       <div className="sticky top-4 z-20 flex flex-col gap-2 bg-zinc-900/90 backdrop-blur-xl p-3 rounded-2xl border border-white/5 shadow-2xl">
            <div className="flex justify-between items-center px-1">
                <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest ml-1">{t('filter_verification_zone')}</span>
                <Button 
                    size="sm" 
                    variant="light" 
            ...
            <Autocomplete 
                placeholder={userLocation ? "Using Current Location" : t('search_city_placeholder')}
                ...
                startContent={
                    <div className="flex items-center justify-center min-w-8 text-zinc-500 pl-2 mr-1">
                        {userLocation ? <LocateFixed size={18} className="text-emerald-500" /> : <Search size={18} />}
                    </div>
                }
                inputProps={{
                    classNames: {
                        input: "w-full !bg-transparent !outline-none !ring-0 !ring-offset-0 focus:!ring-0 placeholder:text-zinc-500 text-zinc-100",
                        inputWrapper: "flex flex-row items-center !bg-zinc-950/50 border border-zinc-800 !shadow-none !outline-none !ring-0 !ring-offset-0 focus-within:!ring-0 focus-within:!border-zinc-700 !min-h-11 h-11 rounded-xl transition-colors",
                ...
                popoverProps={{
                    classNames: {
                        content: "bg-zinc-950 border border-zinc-800 p-0 rounded-xl shadow-xl"
                    }
                }}
            >
                {filteredCities.map((city) => (
                    <AutocompleteItem key={city.n} textValue={getCityDisplayName(city)} className="text-zinc-300">
       ...
       <div className="flex flex-col gap-3">
           {loading ? (
               <div className="flex flex-col items-center justify-center py-20 gap-4 opacity-50">
                    ...
                    <span className="text-xs text-zinc-500 font-mono tracking-widest uppercase">{t('loading_scanning')}</span>
               </div>
           ) : sortedResources.length === 0 ? (
               <div className="text-center py-12 px-6 border border-dashed border-zinc-800 rounded-3xl bg-zinc-900/20">
                   <p className="text-zinc-500 font-medium">{t('no_resources_found')}</p>
                   <p className="text-xs text-zinc-600 mt-2">{t('try_city_suggestion')}</p>
               </div>
           ) : (
               sortedResources.map((res) => {
                   ...
                   if (activeLocation && rLat && rLng) {
                       const d = calculateDistance(activeLocation.lat, activeLocation.lng, rLat, rLng);
                   ...
                   return (
                       ...
                       >
                           <div className="flex justify-between items-start">
                               <div className={`p-2 rounded-lg ${CATEGORY_COLORS[res.category] || "text-zinc-500 bg-zinc-900"} border border-zinc-900/50`}>
                                   <Icon size={20} />
                               </div>
                               
                               <div className="flex items-center gap-3">
                                   {activeLocation && distance && (
                                        <span className="text-[9px] font-bold font-mono text-emerald-500 bg-emerald-500/10 h-6 px-2 flex items-center justify-center rounded-full border border-emerald-500/20">{distance}</span>
                                   )}
                                   <Button 
                                    size="sm" 
                                    isIconOnly 
                           ...
                           <div className="flex flex-col gap-1">
                               <div className="flex items-center gap-2">
                                   <span className="font-bold text-sm text-zinc-200">{res.name}</span>
                                   {res.verified && <ShieldCheck size={12} className="text-emerald-500" />}
                               </div>
                               <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">{t(`cat_${res.category}` as any)}</span>
